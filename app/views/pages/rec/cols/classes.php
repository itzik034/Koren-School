...
<script src="assets/js/rec/cols/classes.js" defer></script>
...
if(!isset($_GET['course_id'])){die('error');}
...
{
    $sql9 = "SELECT * FROM classes WHERE class_status = 'publish' AND course_id = '$course_id' ORDER BY id DESC";
}
...
if(mysqli_num_rows($query9) > 0)
{
    ...
    while($row = mysqli_fetch_array($query9))
    {
        ...
        $c_text .= '</div>';
        $c_text .= '<div class="rc_text2_prog dash_quiz_box_prog" id="rc_prog_inside_'.$row['id'].'">';
...
{
    $c_text = "<span style='padding:10px;'>עדיין אין כיתות בקורס הזה.</span>";
}
