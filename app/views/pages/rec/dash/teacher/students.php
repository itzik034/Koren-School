<script src="assets/js/rec/dash/teacher/students.js"></script>
...
include_once(__DIR__."../../../../../action/connection.php");
include_once(__DIR__."../../../../../action/function.php");
...
if(mysqli_num_rows($query3) > 0)
{
    ...
    while($row = mysqli_fetch_array($query3))
    {
        ...
        if(check_student_teacher($conn, $row['id'], $user_id))
        {
            ...
            $s_text .= '<div class="rc_text2_num std_num_';
            $s_text .= $student_id;
            $s_text .= '"></div>';
            $s_text .= '<div class="rc_text2_prog dash_quiz_box_prog">';
            $s_text .= '<span class="rc_prog_inside std_prog_in_tch_dash_';
            $s_text .= $student_id;
            $s_text .= '" title="ממוצע התקדמות של התלמיד בשאלונים"></span>';
            $s_text .= '</div>';
            $s_text .= '</div>';
            $s_text .= '</div>';
            $s_text .= '</div>';
    ...
    if($s_text == "")
    {
        echo "<span style='padding:10px;'>עדיין אין לך תלמידים.</span>";
    }
}
else
{
