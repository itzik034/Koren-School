$(document).ready(function()
{
    ...
    // Load the classes list to top toggle
    update_classes_in_dash();
    ...
    window.update_teacher_dash_cols = function()
    {
        get_current_class((current_class) => 
        {
            $.get("layout/rec/dash/teacher/cols/classes.php?class_id="+current_class, (res) => 
            {
                $(".rc_col_classes").html(res);
            });
            
            if(current_class === '-1') { return }

            $.get("layout/rec/dash/teacher/cols/quizzes.php?class_id="+current_class, (res) => 
            {
                $(".rc_col_quiz").html(res);
            });

            $.get("layout/rec/dash/teacher/cols/students.php?class_id="+current_class, (res) => 
            {
